...
import { FaHouse, FaPencil, FaPenToSquare, FaPlus, FaTrash } from 'react-icons/fa6';
...
const saveFormData = async () => {
  try {
    ...
    console.log(_doc);
    await setDoc(doc(db, "users", user?.uid, "resumes", resume_id), _doc);
...
return (
    <div className="w-full flex flex-col items-center justify-start gap-4">
      {/* Breadcrumb */}
      <div className="w-full flex items-center gap-2 px-4 py-2 bg-gray-100">
        <Link
          to="/"
          className="flex items-center justify-center gap-2 text-txtPrimary hover:text-txtPrimaryHover"
        >
          <FaHouse />
          Home
        </Link>
        <span className="text-txtPrimary">/</span>
        <p
          className="text-txtPrimary cursor-pointer hover:text-txtPrimaryHover"
          onClick={() => navigate(-1)}
        >
          Template1
        </p>
        <span className="text-txtPrimary">/</span>
        <p className="text-txtSecondary">Edit</p>
      </div>
