               ...
               onClick={()=>setIsMenu(!isMenu)}>
                ...
                flex flex-col items-center
                 ...
                 onMouseLeave={()=>setIsMenu(false)}>
                  {data.photoURL ? (
                  <div className='w-20 h-20 rounded-full relative flex-col
                  ...
                  <div className='w-20 h-20 rounded-full relative flex items-center justify-center bg-blue-700 shadow-md cursor-pointer'>
                    <p className='text-3xl text-white'>{data?.email[0]}</p>
                      ...
                      {data?.displayName &&   
                                       <p className='text-lg text-txtDark'>{data?.displayName}
                                       </p>}
                                       {/* menus */}
                                       ...
                                       <Link className='text-txtLight hover:text-txtDark text-base whitespace-nowrap' to={"/profile"}>My Account</Link>
{
  adminIds.includes(data?.uid) &&(
    <Link className='text-txtLight hover:text-txtDark text-base whitespace-nowrap' to={"/template/create"}>Add New Template</Link>
                                        ...
                                        border-gray-300 flex items-center justify-between group">
                                          <p className='group-hover:text-txtDark text-txtLight cursor-pointer' onClick={signOutUser}>Sign Out</p>
                                          <HiLogout className='group-hover:text-txtDark text-txtLight cursor-pointer' onClick={signOutUser}/>
                                        </div>
                                       </div>
                 </motion.div>}
                </AnimatePresence>
              ...
              <Link to="/auth">
                <motion.button className='px-4 py-2 rounded-md border border-gray-300 bg-gray-200
                hover:shadow-md active:scale-95
                duration-150' type='button' {...FadeInOutWithOpacity}>
                  Login
                </motion.button>
              </Link>
            )}
